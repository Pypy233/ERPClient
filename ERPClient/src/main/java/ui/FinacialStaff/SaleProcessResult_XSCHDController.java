...
public class SaleProcessResult_XSCHDController implements Initializable{
    private Main main;
    ...
    public SaleProcessResult_XSCHDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<SaleVO> list){
        this.mainList=list;
        ...
        label_type.setText("销售出货单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(SaleVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("销售出货单",saleVO.getNumber());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(SaleVO vo: mainList){
                        if(model.getNumber().equals(vo.getNumber())){
                            main.gotoSaleProcessDetail_XSCHD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
