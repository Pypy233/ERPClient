...
public class SaleProcessResult_JHTHDController implements Initializable {
    private Main main;
    ...
    public SaleProcessResult_JHTHDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<StockReturnVO> list){
        ...
        label_type.setText("进货退货单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(StockReturnVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("进货退货单",saleVO.getNumber());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(StockReturnVO vo: mainList){
                        if(model.getNumber().equals(vo.getNumber())){
                            main.gotoSaleProcessDetail_JHTHD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
