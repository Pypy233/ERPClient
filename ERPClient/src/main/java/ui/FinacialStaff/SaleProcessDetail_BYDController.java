...
public class SaleProcessDetail_BYDController implements Initializable{
    private Main main;
    private UserVO userVO;
    private OverflowListVO overflowListVO;
    private ArrayList<OverflowListVO> mainList;
    ...
    public SaleProcessDetail_BYDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,OverflowListVO overflowlistvo,ArrayList<OverflowListVO> arrayList){
        this.main=main;
        this.userVO=uservo;
        this.overflowListVO=overflowlistvo;
        ...
        label_number.setText(""+overflowListVO.getId());
        label_date.setText(overflowListVO.getDate());
        label_name.setText(userVO.getName());

        tf_operator.setText("4");

        Set<GoodsOverflowVO> set=overflowListVO.getSet();

        ObservableList<BYDModel> data = FXCollections.observableArrayList();

        for(GoodsOverflowVO vo:set){
            BYDModel model = new BYDModel(vo.getVo().getNumber()+"",vo.getVo().getName(),
                    vo.getVo().getType(),vo.getTrueNumber()+"",vo.getVo().getPurchasePrice()+"");
            data.add(model);
        }
    ...
    public void red() throws RemoteException{
        OverflowListVO temp=helper.getOverflowListBLService().addOverflowListRed(overflowListVO);
    ...
    public void redAndCopy() throws RemoteException{
        OverflowListVO overflowlistvo = helper.getOverflowListBLService().addOverflowListRed(overflowListVO);
        AlertUtil.showInformationAlert("红冲成功！");
    ...
    public void gotoSaleProcessResult(){
        main.gotoSaleProcessResult_BYD(userVO,mainList);
    }
    ...
    public void out() throws Exception{
        ...
        ExcelTools.export("报溢单"+date.getTime(),a);
