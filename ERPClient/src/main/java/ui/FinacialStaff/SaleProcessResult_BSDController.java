...
public class SaleProcessResult_BSDController implements Initializable{
    private Main main;
    ...
    public SaleProcessResult_BSDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<LackListVO> list){
        this.mainList=list;
        ...
        label_type.setText("报损单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(LackListVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("报损单",""+saleVO.getId());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(LackListVO vo: mainList){
                        if(model.getNumber().equals(vo.getId()+"")){
                            main.gotoSaleProcessDetail_BSD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
