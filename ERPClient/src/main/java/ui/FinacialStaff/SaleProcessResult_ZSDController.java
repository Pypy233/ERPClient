...
public class SaleProcessResult_ZSDController implements Initializable{
    private Main main;
    ...
    public SaleProcessResult_ZSDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<PresentListVO> list){
        this.mainList=list;
        ...
        label_type.setText("赠送单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(PresentListVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("赠送单",""+saleVO.getId());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(PresentListVO vo: mainList){
                        if(model.getNumber().equals(vo.getId()+"")){
                            main.gotoSaleProcessDetail_ZSD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
