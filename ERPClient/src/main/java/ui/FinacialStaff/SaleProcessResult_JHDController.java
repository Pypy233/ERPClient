...
public class SaleProcessResult_JHDController implements Initializable{
    private Main main;
    ...
    public SaleProcessResult_JHDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<StockVO> list){
        this.mainList=list;
        ...
        label_type.setText("进货单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(StockVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("进货单",saleVO.getNumber());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(StockVO vo: mainList){
                        if(model.getNumber().equals(vo.getNumber())){
                            main.gotoSaleProcessDetail_JHD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
