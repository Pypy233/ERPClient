...
public class SaleProcessDetail_JHDController implements Initializable{
    private Main main;
    private UserVO userVO;
    private ArrayList<StockVO> mainList;
    ...
    public SaleProcessDetail_JHDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,StockVO stockvo , ArrayList<StockVO> arrayList){
        this.main=main;
        this.userVO=uservo;
        ...
        label_sum.setText("￥"+stockvo.getTotalPrice()+"");
        ...
        StockVO temp = helper.getStockBLService().addStockRed(stockVO);
        ...
        StockVO stockvo = helper.getStockBLService().addStockRed(stockVO);
        AlertUtil.showInformationAlert("红冲成功！");
            ...
            for(GoodsStockVO vo:set){
                SaleProcessDetail_XSLDJModel model = new SaleProcessDetail_XSLDJModel(vo.getVo().getNumber()+"",vo.getVo().getName(),vo.getVo().getType(),
                        vo.getStockNumber()+"",vo.getVo().getPurchasePrice()+"",vo.getTotalPrice()+"",vo.getRemark());
                data.add(model);
            }
    ...
    public void gotoSaleProcessResult(){
        main.gotoSaleProcessResult_JHD(userVO,mainList);
            }
    ...
    public void out() throws Exception{
        ...
        ExcelTools.export("进货单"+date.getTime(),a);
