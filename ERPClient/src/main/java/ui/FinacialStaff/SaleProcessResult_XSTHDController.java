...
public class SaleProcessResult_XSTHDController implements Initializable{
    private Main main;
    ...
    public SaleProcessResult_XSTHDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<SaleReturnVO> list){
        this.mainList=list;
        ...
        label_type.setText("销售退货单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(SaleReturnVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("销售退货单",saleVO.getNumber());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(SaleReturnVO vo: mainList){
                        if(model.getNumber().equals(vo.getNumber())){
                            main.gotoSaleProcessDetail_XSTHD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
