...
public class SaleProcessResult_BYDController implements Initializable{
    private Main main;
    ...
    public SaleProcessResult_BYDController(){

    }//一个构造函数

    public void setMain(Main main,UserVO uservo,ArrayList<OverflowListVO> list){
        this.mainList=list;
        ...
        label_type.setText("报溢单");

        ObservableList<SaleProcessResultModel> data = FXCollections.observableArrayList();
        for(OverflowListVO saleVO:list){
            SaleProcessResultModel model=new SaleProcessResultModel("报溢单",""+saleVO.getId());
            data.add(model);
        }
    ...
    public void gotoSaleProcessResultDetail(){
        ...
        if(counter==0)
            ...
            for(SaleProcessResultModel model : data){
                if(model.checkBox.isSelected()){
                    for(OverflowListVO vo: mainList){
                        if(model.getNumber().equals(vo.getId()+"")){
                            main.gotoSaleProcessDetail_BYD(userVO,vo,mainList);
                            break;
                        }
                    }
                }
