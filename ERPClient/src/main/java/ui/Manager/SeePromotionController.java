...
public class SeePromotionController implements Initializable{
    ...
    public SeePromotionController(){

    }//一个构造函数

    public void setMain (Main main,UserVO uservo,PromotionVO promotionvo){
        this.main=main;
        this.userVO=uservo;
        ...
        ArrayList<BargainSet> bargainSets = promotionvo.getBargainList();
        ArrayList<PromotionStrategy> strategies = promotionvo.getStrategyList();

        for(BargainSet set:bargainSets){
            BargainModel model;
            if(set.getGoodsList().size()==0)
                model = new BargainModel("","","","","","","");
            else if(set.getGoodsList().size()==1)
                model = new BargainModel(set.getGoodsList().get(0).getName(),set.getGoodsList().get(0).getQuantity()+"",
                        "","","","",set.getDiscount()+"");
            else if(set.getGoodsList().size()==2)
                model = new BargainModel(set.getGoodsList().get(0).getName(),set.getGoodsList().get(0).getQuantity()+"",
                        set.getGoodsList().get(1).getName(),set.getGoodsList().get(1).getQuantity()+"","" ,"",set.getDiscount()+"" );
            else
                model = new BargainModel(set.getGoodsList().get(0).getName(),set.getGoodsList().get(0).getQuantity()+"",
                        set.getGoodsList().get(1).getName(),set.getGoodsList().get(1).getQuantity()+"",set.getGoodsList().get(2).getName() ,set.getGoodsList().get(2).getQuantity()+"",set.getDiscount()+"");
            data1.add(model);
            }
        for(PromotionStrategy strategy:strategies){
            PromotionModel model = new PromotionModel(strategy.getGiftList().get(0).getName(),strategy.getGiftList().get(0).getQuantity()+"",
                    strategy.getGiftList().get(1).getName(),strategy.getGiftList().get(1).getQuantity()+"",
                    strategy.getGiftList().get(2).getName(),strategy.getGiftList().get(2).getQuantity()+"",strategy.getCharge_max()+"",
                    strategy.getCharge_min()+"",strategy.getCoupon()+"",strategy.getDiscount()+"");
            ArrayList<Integer> temp = strategy.getLevelList();
            for(Integer num:temp){
                if(num==1)
                    model.level1.setSelected(true);
                else if(num==2)
                    model.level2.setSelected(true);
                else if(num==3)
                    model.level3.setSelected(true);
                else if(num==4)
                    model.level4.setSelected(true);
                else
                    model.level5.setSelected(true);
            }
            data2.add(model);
        }
        ...
        LocalDate d1=LocalDate.parse((CharSequence)promotionvo.getBegin());
        LocalDate d2 = LocalDate.parse((CharSequence)promotionvo.getEnd());
        from.setValue(d1);
        to.setValue(d2);
    ...
    public void confirm() throws RemoteException,MalformedURLException,NotBoundException{
        ...
        ResultMessage message = helper.getPromotionBLService().update(vo);
    ...
    public void delete() throws RemoteException,MalformedURLException,NotBoundException{
        helper.getPromotionBLService().delete(promotionVO);
